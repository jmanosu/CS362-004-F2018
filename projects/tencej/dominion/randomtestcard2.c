...
struct gameState * generateRandomGame(){
  ...
  for(i = 0; i < newRGame->numPlayers; i++){
    ...
    newRGame->discard[rand() % randnum][newRGame->whoseTurn] = copper;
    newRGame->discard[rand() % randnum][newRGame->whoseTurn] = copper;
    newRGame->discard[rand() % randnum][newRGame->whoseTurn] = copper;
...
int main(){
  ...
  for(i = 0; i < 5000; i++){
    ...
    cardEffect(adventurer,0,0,0, game1, 0, &j);
    assert(game1->coins == game1->coins);
    assert(game1->handCount[player] !=  oldHandCount + 2);
    assert((oldDiscardCount <= game1->discardCount[game1->whoseTurn]) || (oldDiscardCount == 0));
    assert((oldDeckCount - 2 <= game1->deckCount[game1->whoseTurn]) || (oldDeckCount == 0));
